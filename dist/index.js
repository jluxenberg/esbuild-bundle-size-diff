...
var comment = (message) => __async(exports, null, function* () {
...
var computeSizeByEntrypoint = (metafile) => {
  const result = {};
  Object.entries(metafile.outputs).forEach(([path2, output]) => {
    const { entryPoint, bytes, cssBundle } = output;
    if (!entryPoint)
      return;
    result[entryPoint] || (result[entryPoint] = []);
    result[entryPoint].push({ path: path2, bytes });
    if (cssBundle) {
      const cssOutput = metafile.outputs[cssBundle];
      if (cssOutput) {
        if (!result[entryPoint].find(({ path: path3 }) => path3 === cssBundle)) {
...
var diff = (baseMetafile, prMetafile) => {
  ...
  entrypoints.forEach((entrypoint) => {
    var _a, _b;
    const baseSize = (_a = baseSizes[entrypoint]) != null ? _a : [];
    const prSize = (_b = prSizes[entrypoint]) != null ? _b : [];
    ...
    const paths = [
      .../* @__PURE__ */ new Set([
        ...baseSize.map(({ path: path2 }) => path2),
        ...prSize.map(({ path: path2 }) => path2)
      ])
    ].sort();
    paths.forEach((path2) => {
      const baseItem = baseSize.find(({ path: p }) => p === path2);
      const prItem = prSize.find(({ path: p }) => p === path2);
      const diffSizeTotal2 = baseItem !== void 0 && prItem !== void 0 ? prItem.bytes - baseItem.bytes : 0;
      result.push(
        [
          "",
          path2,
          (baseItem == null ? void 0 : baseItem.bytes) !== void 0 ? bytesToSize(baseItem == null ? void 0 : baseItem.bytes) : "n/a",
          (prItem == null ? void 0 : prItem.bytes) !== void 0 ? bytesToSize(prItem == null ? void 0 : prItem.bytes) : "n/a",
          baseItem === void 0 ? "new" : prItem === void 0 ? "deleted" : `${bytesToSize(diffSizeTotal2)} (${percentChange.toFixed(2)}%)`
        ].map((str) => "<sub>" + str + "</sub>")
        // <sub> makes the text smaller, h/t https://stackoverflow.com/questions/66380290/how-to-use-smaller-font-size-in-a-github-table
      );
    });
  });
  return result.map((row) => "| " + row.map(escapePipes).join(" | ") + " |").join("\n");
};
var run = () => __async(exports, null, function* () {
  ...
  const baseMetafile = JSON.parse(import_fs.default.readFileSync(base, "utf8"));
  const prMetafile = JSON.parse(import_fs.default.readFileSync(pr, "utf8"));
...
var main = (base, pr) => __async(exports, null, function* () {
  const baseMetafile = JSON.parse(import_fs.default.readFileSync(base, "utf8"));
  const prMetafile = JSON.parse(import_fs.default.readFileSync(pr, "utf8"));
  const resultMarkdownTable = diff(baseMetafile, prMetafile);
